//总结：二叉树的遍历总结（前序、中序、后序）

/*
 * 【总结：完全二叉树的存储结构可以为顺序存储结构（数组）一般的二叉树也可以用顺序存储结构但浪费空间】
 * 【总结：若使用链表存储，则有两个指针】
 * 【总结：二叉树遍历核心问题是二叉树结构线性化，层序遍历采用队列存储结构】
 * 1、求所有叶子节点：前序遍历前面加一个条件：if(!node->left && !node->right)
 * 2、求二叉树高度：a.求左子树高度 b.求右子树高度 c.max(L, R) + 1
 * 3、确认二叉树，必须要有中序遍历
 * /
